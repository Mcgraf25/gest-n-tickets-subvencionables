...
import { extractReceiptInfo, ExtractedReceipt } from './services/geminiService';
import type { CaseFile, Receipt, ReceiptItem, Eligibility } from './types';
...
const App: React.FC = () => {
    ...
    const currentCaseFile = useMemo(() => caseFiles.find(cf => cf.id === currentCaseFileId), [caseFiles, currentCaseFileId]);

    const updateCaseFile = (updatedFile: Partial<CaseFile>) => {
        if (!currentCaseFileId) return;
        setCaseFiles(prev => prev.map(cf => cf.id === currentCaseFileId ? { ...cf, ...updatedFile } : cf));
    };
        ...
        return new Promise((resolve, reject) => {
            reader.onload = async (e) => {
                try {
                    ...
                    const extractedData = await extractReceiptInfo(base64Content, mimeType, guidelines);
    ...
    const handleProcessReceipts = async (files: File[]) => {
        ...
        try {
            ...
            const newReceipts = (await Promise.all(receiptPromises)).filter((r): r is Receipt => r !== null);
            
            if (newReceipts.length > 0) {
                updateCaseFile({ receipts: [...(currentCaseFile?.receipts || []), ...newReceipts] });
            }

            if (newReceipts.length < files.length) {
                alert(`Se procesaron ${newReceipts.length} de ${files.length} recibos. Algunos archivos no pudieron ser procesados.`);
            }
    ...
    const handleReprocessReceipt = async (receiptId: string) => {
        ...
        try {
            const extractedData = await extractReceiptInfo(receipt.base64Content, receipt.mimeType, guidelines);
            if (extractedData) {
                ...
                updateCaseFile({ receipts: currentCaseFile?.receipts.map(r => r.id === receiptId ? updatedReceipt : r) || [] });
            } else {
    ...
    const handleEligibilityChange = (itemId: string, newEligibility: Eligibility) => {
        const updatedReceipts = currentCaseFile?.receipts.map(r => ({
            ...r,
            items: r.items.map(item => item.id === itemId ? { ...item, manualEligibility: newEligibility } : item)
        })) || [];
    ...
    const handleResetCaseData = () => {
        ...
        if (window.confirm(`¿Estás seguro? Se eliminarán TODOS los recibos del expediente "${currentCaseFile.name}". Esta acción no se puede deshacer.`)) {
            // Create a new array of case files with the target one updated
            const updatedCaseFiles = caseFiles.map(cf => {
                if (cf.id === currentCaseFileId) {
                    // Return a new object for the case file with receipts cleared
                    return { ...cf, receipts: [] };
                }
                // Return all other case files unchanged
                return cf;
            });
    ...
    const handleDeleteCaseFile = () => {
        ...
        if (window.confirm(`¿Estás seguro de que quieres ELIMINAR el expediente "${caseNameToDelete}"? Esta acción es permanente y no se puede deshacer.`)) {
            ...
            setCurrentCaseFileId(newCaseFiles.length > 0 ? newCaseFiles[newActiveIndex].id : null);
    ...
    const allItems = useMemo(() => {
        return currentCaseFile?.receipts.flatMap(r => r.items.map(item => ({
            ...item,
    ...
    const renderContent = () => {
        ...
        switch (activeTab) {
            ...
            case 'upload':
                return <ReceiptUploader onProcessReceipts={handleProcessReceipts} isLoading={isLoading} />;
            case 'list':
                return <ReceiptList 
                            receipts={currentCaseFile.receipts} 
                            onViewReceipt={(id) => setViewingReceipt(currentCaseFile.receipts.find(r => r.id === id) || null)}
...
const TabButton: React.FC<{ tab: Tab; label: string; icon: React.ReactNode; activeTab: Tab; setActiveTab: (tab: Tab) => void; }> = 
({ tab, label, icon, activeTab, setActiveTab }) => (
    <button
        onClick={() => setActiveTab(tab)}
        className={`w-full flex items-center space-x-3 px-3 py-2 text-sm font-medium rounded-lg transition-colors ${activeTab === tab ? 'bg-primary text-white' : 'text-slate-600 hover:bg-slate-100'}`}
    >
        {icon}
